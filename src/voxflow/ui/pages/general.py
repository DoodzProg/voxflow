...
def _startup_cmd() -> str:
    ...
    Returns:
        Quoted executable path; uses ``-m voxflow.ui.app`` when running as a
        plain Python interpreter (development mode).
    """
    if getattr(sys, "frozen", False):
        return f'"{sys.executable}"'
    return f'"{sys.executable}" -m voxflow.ui.app'


def _is_startup_enabled() -> bool:
    """Check whether Voxflow is registered to run at Windows startup.
...
def _set_startup(enabled: bool) -> None:
    """Add or remove the Voxflow startup registry entry.
...
class GeneralPage(BasePage):
    ...
    def _refresh_icon_label(
        ...
        Args:
            ...
            t:      Active theme.
        """
        from voxflow.ui.components import _svg_data  # noqa: PLC0415
