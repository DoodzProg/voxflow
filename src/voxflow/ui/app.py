"""
src/voxflow/ui/app.py

Voxflow main window, floating voice overlay and application entry point.

Architecture
------------
- :class:`VoiceOverlay`  — frameless floating widget shown during dictation.
- :class:`VoxflowApp`    — top-level :class:`~PySide6.QtWidgets.QMainWindow`;
  owns navigation, theme switching, dictation lifecycle and history management.

Theme switching
---------------
:meth:`VoxflowApp.apply_theme` is the **single** entry point for theme changes.
It updates the global theme state (via :func:`~voxflow.ui.components.set_theme`),
applies a new :class:`~PySide6.QtGui.QPalette` + application-level QSS, updates
all sidebar widgets in-place, then calls :meth:`~voxflow.ui.pages.base.BasePage.retheme`
on every registered page — preserving all user-input state.
"""
...
from voxflow.core.engine import DictationEngine
...
class VoxflowApp(QMainWindow):
    ...
    Dictation sessions are driven by :class:`~voxflow.core.hotkey.HotkeyListener`
    and :class:`~voxflow.core.engine.DictationEngine`.
    ...
    def _build_tray(self) -> None:
        ...
        quit_action = tray_menu.addAction("Quitter Voxflow")
    ...
    def _toggle_theme(self) -> None:
        ...
        self.apply_theme(dark=not self._is_dark)

    def apply_theme(self, dark: bool) -> None:
        """Switch the entire application to the given theme mode.

        Updates:
        - Module-level theme state (via :func:`~voxflow.ui.components.set_theme`).
        - Application :class:`~PySide6.QtGui.QPalette` and global QSS.
        - All sidebar widgets (logo, nav buttons, status bar, action buttons).
        - Every registered :class:`~voxflow.ui.pages.base.BasePage` via ``retheme()``.
        - Dynamic dashboard content via :meth:`refresh_dashboard` and
          :meth:`refresh_home_shortcuts`.
        ...
        # Rebuild dynamic content (already reads _t() internally)
    ...
    def _add_shortcut_card(
        ...
        Args:
            ...
            icon:       Icon key from :data:`~voxflow.ui.styles.ICONS`.
            t:          Active theme.
        """
        from PySide6.QtWidgets import QFrame  # noqa: PLC0415
        sc = QFrame()
