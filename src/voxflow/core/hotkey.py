"""
src/voxflow/core/hotkey.py

Hotkey listener for the Voxflow application.
...
Both hotkeys support two trigger modes stored in :class:`~voxflow.utils.config.ConfigManager`:
...
class HotkeyListener(QThread):
    """Background thread that monitors global keyboard state for Voxflow hotkeys.
    ...
    def run(self) -> None:
        ...
        and ``HOTKEY_CONTEXT_MODE`` from :class:`~voxflow.utils.config.ConfigManager`
        on every iteration so that hotkey changes take effect immediately without
