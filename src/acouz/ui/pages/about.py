...
src/acouz/ui/pages/about.py

About page for the AcouZ application.
...
from PySide6.QtCore import Qt, QThread, Signal, QByteArray, QUrl
from PySide6.QtGui import QDesktopServices
from PySide6.QtSvgWidgets import QSvgWidget

import acouz.ui.styles as S
from acouz.ui.styles import Theme, LOGO_SVG
...
_CURRENT_VERSION = "1.0.2"
_REPO = "DoodzProg/acouz"
        ...
        try:
            req = urllib.request.Request(
                ...
                headers={"Accept": "application/vnd.github+json",
                         "User-Agent": "AcouZ-UpdateChecker/1.0"},
            )
            ...
            if tag and tag != _CURRENT_VERSION:
                self.result.emit(tr("about.update.available").format(tag=tag), tag)
            else:
...
class AboutPage(BasePage):
    ...
    def _check_updates(self) -> None:
        """Start a background thread or open the release page if update is known."""
        if self._pending_update_tag:
            # Update already found — open the release page directly
            url = f"https://github.com/{_REPO}/releases/tag/v{self._pending_update_tag}"
    ...
    def _on_update_result(self, message: str, tag: str) -> None:
        """Display the update-check result on the button.

        Args:
            message: Human-readable status string from :class:`_UpdateChecker`.
            tag:     New version tag if available, empty string otherwise.
        """
        self._pending_update_tag = tag
        self._update_btn.setEnabled(True)
        if tag:
            # Show download-style button when update is available
            self._update_btn.setText(f"⬇ {message}")
            self._update_btn.setStyleSheet(
                "QPushButton { color: #10B981; border: 1px solid #10B981; "
                "background: rgba(16,185,129,0.10); border-radius: 8px; "
                "padding: 6px 16px; font-size: 13px; font-weight: 600; } "
                "QPushButton:hover { background: rgba(16,185,129,0.20); }"
            )
        else:
